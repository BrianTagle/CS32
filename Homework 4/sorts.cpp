...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
	...
	{
		int j = i;
		Sensor currentItem = s[i];
		while (j > 0 && comp(currentItem, s[j-1])) 
		{
			s[j] = s[j - 1];
			j--;
		}
...
int main()
{
	...
	// destruction time.
	{
		...
		//        order.
		for (int i = 0; i < SensorPT.size(); i++) {
			sensors[i] = *(SensorPT[i]);
		}
